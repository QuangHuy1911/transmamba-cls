# Training Script for Pure Mamba Baseline
# Usage: python train_mamba_baseline.py --task sst2 --epochs 5
...
from models.mamba_baseline import PureMambaClassifier
...
def main():
    parser = argparse.ArgumentParser(description="Train Pure Mamba Baseline")
    ...
    print("=" * 60)
    print(f"Pure Mamba Baseline Training")
    ...
    model = PureMambaClassifier(
        vocab_size=30522, d_model=args.d_model, n_layers=args.n_layers,
        num_labels=num_labels, max_length=args.max_length,
    ).to(device)
    ...
    output_dir = f"results/mamba_baseline_{args.task}"
    ...
    with open(f"{output_dir}/results.json", "w") as f:
        json.dump({
            "model": "Pure Mamba Baseline", "task": args.task, "best_val_acc": best_acc,
            "total_params": model.count_parameters(), "total_time_minutes": total_time / 60,
